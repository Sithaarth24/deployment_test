...
def generate_grid():
    ...
    # Move one level up from 'gridGeneration' to the main folder
    main_dir = os.path.dirname(script_dir)

    # Define the path to the 'objects' folder
    pkl_file_path = os.path.join(main_dir, 'objects')

    # Open the file using the corrected path
    with open(pkl_file_path +'\\' + 'num_cols_per_row2.pkl', 'rb') as f:
        num_cols_per_row = pickle.load(f)

    with open(pkl_file_path + '\\' + 'all_points2.pkl', 'rb') as f:
        all_points = pickle.load(f)

    with open(pkl_file_path + '\\' + 'grid_cells2.pkl', 'rb') as f:
        grid_cells = pickle.load(f)

    with open(pkl_file_path + '\\' + 'points_to_ind2.pkl', 'rb') as f:
        points_to_ind = pickle.load(f)

    with open(pkl_file_path + '\\' + 'grid2.pkl', 'rb') as f:
        grid = pickle.load(f)

    with open(pkl_file_path + '\\' + 'points_to_bathy2.pkl', 'rb') as f:
...
def build_KDTree(all_points):
    ...
    kdtree = None
    with open(pkl_file_path + '\\' + 'kdtree_file2.pkl', 'rb') as f:
